...
import { createDevelopmentMintz, DEVELOPMENT_MINT_REFERENCE_TIME, getDevelopmentMintById } from "@/data/development/mintz";
import { CURRENT_DEVELOPMENT_USER_ID, getDevelopmentUserById } from "@/data/development/users";
...
export default async function MintRoute({ params }: { params: Promise<{ mintId: string }> }) {
  ...
  const visible = canViewMint({ mint, viewer, author, friendshipStatus: "none", viewerFollowsAuthor: false, authorFollowsViewer: false, blocked: false, currentTime })
    && (!organization || canViewOrganizationContent({ id: viewer.account.id, universityId: viewer.account.universityId }, organization.id, mint.organizationAudience, developmentOrganizationMemberships));
  ...
  const theme = universities[viewer.account.universityId];
