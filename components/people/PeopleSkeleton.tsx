...
export function PeopleSkeleton({ viewer, theme, profiles, onOpenProfile }: PeopleSkeletonProps) {
  ...
  return (
    <div className="space-y-5">
      <div className="px-1"><p className="text-[10px] font-black uppercase tracking-[0.22em] text-slate-400">Campus discovery</p><h1 className="mt-1 text-3xl font-black tracking-tight text-slate-950">People</h1><p className="mt-2 max-w-2xl text-sm text-slate-600">Find friends, classmates, roommates, and collaborators without exposing private academic details.</p></div>
      <label className="block"><span className="sr-only">Search people</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search people, majors, interests…" className="w-full rounded-2xl border border-white/80 bg-white/90 px-4 py-3.5 text-sm shadow-sm outline-none focus:ring-2" style={{ caretColor: theme.primary }} /></label>
