...
export function ClubsDiscovery({ user, theme, organizations, onMembershipAction }: ClubsDiscoveryProps) {
  const visible = developmentOrganizations.filter((organization) => theme.accessibleCampuses.includes(organization.universityId));
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{visible.map((organization) => {
        ...
        const actionable = canJoinOrganization(user, organization) && !["member", "officer", "leader", "requested", "blocked"].includes(status);
