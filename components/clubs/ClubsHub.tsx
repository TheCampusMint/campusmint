...
export function ClubsHub({ user, viewer, profiles, theme, events, stories, organizations }: ClubsHubProps) {
  ...
  const rankedEvents = rankEventContent(events);
  const clubEvents = rankedEvents.filter((event) => event.organizationId && directoryIds.has(event.organizationId));
  ...
  function nextEventFor(organizationId: string) {
    return rankedEvents.find((event) => event.organizationId === organizationId);
  }
