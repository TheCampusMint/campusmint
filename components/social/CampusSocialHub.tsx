...
export function CampusSocialHub({ viewer, user, theme, profiles, mintz, organizations, stories, currentTime, onToggleStoryLike, onAddStoryComment, onCreateStory, onOpenProfile, onMembershipAction, onOpenClubs }: CampusSocialHubProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {createOpen && <CreateContentFlow viewer={viewer} users={users} theme={theme} onCreateMint={mintz.createMint} onClose={() => setCreateOpen(false)} organizationMemberships={organizations.memberships} organizationRoles={organizations.roles} />}
    </div>
  );
}
