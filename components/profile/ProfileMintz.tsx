...
export function ProfileMintz({ viewer, owner, theme, profiles, mintz, organizations, onOpenProfile }: ProfileMintzProps) {
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex gap-2 border-b border-slate-100 pb-4"><button type="button" onClick={() => setTab("mintz")} className="rounded-lg px-4 py-2 text-sm font-bold" style={tab === "mintz" ? { backgroundColor: theme.primary, color: theme.secondary } : { backgroundColor: "#f1f5f9", color: "#475569" }}>Mintz</button><button type="button" onClick={() => setTab("tagged")} className="rounded-lg px-4 py-2 text-sm font-bold" style={tab === "tagged" ? { backgroundColor: theme.primary, color: theme.secondary } : { backgroundColor: "#f1f5f9", color: "#475569" }}>Tagged</button></div>
      {!canViewAccountContent ? <div className="py-8 text-center"><p className="font-black text-slate-900">This account is private.</p><p className="mt-2 text-sm text-slate-500">Follow this person or become friends to view eligible Mintz.</p></div> : visibleMintz.length > 0 ? <div className="mt-5 space-y-5">{visibleMintz.map((item) => {
