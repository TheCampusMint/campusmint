...
export function HousingSkeleton({
  universityId,
  theme,
}: {
  ...
  const listings = universityId
    ? housingEntities.filter(
        (housing) =>
          housing.accessibleUniversityIds.includes(
            universityId,
          ) &&
          (filter === "all" ||
            housing.scope === filter),
      )
    : [];
    ...
    return (
      <div className="rounded-3xl border border-dashed border-slate-300 bg-white/70 p-8 text-center">
        <h2 className="font-black text-slate-900">
          Housing is not configured yet
        </h2>
        <p className="mt-2 text-sm text-slate-500">
          Campus housing for {theme.shortName} will appear
