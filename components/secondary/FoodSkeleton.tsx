...
export function FoodSkeleton({ universityId, theme }: { universityId: UniversityId; theme: UniversityTheme }) {
  const [filter, setFilter] = useState<FoodFilter>("all");
  const locations = diningLocations.filter((location) => location.accessibleUniversityIds.includes(universityId) && (filter === "all" || location.scope === filter));
